...
def _ensure_lesson(
    ...
    *,
    ...
    """Get or create a lesson by slug and return its id."""
    row = conn.execute(
        text("SELECT id FROM lessons WHERE slug = :slug"),
        {"slug": slug},
    ).mappings().first()
...
def _reset_alphabet_1(conn: Connection) -> None:
    """Seed 'alphabet-1' lesson with Ա exercises."""
...
def _reset_alphabet_2(conn: Connection) -> None:
    """Seed 'alphabet-2' lesson with Բ exercises."""
    lesson_id = _ensure_lesson(
        conn,
        slug="alphabet-2",
...
def ensure_lesson_progress_table(engine: Engine) -> None:
    """Create lesson_progress table + unique index if they don't exist."""
    from sqlalchemy import text
    ...
    with engine.begin() as conn:
        ...
        # Unique index (safe against duplicates)
        conn.execute(
            text(
                """
                DO $$
                BEGIN
                    IF NOT EXISTS (
                        ...
                        WHERE c.relname = 'uq_user_lesson_progress'
                    ) THEN
                        CREATE UNIQUE INDEX uq_user_lesson_progress
...
def seed_alphabet_lessons(engine: Engine) -> None:
    """Public entry-point used on startup."""
    with engine.begin() as conn:
