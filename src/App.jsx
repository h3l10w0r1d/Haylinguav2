...
const CmsExerciseLab = lazy(() => import('./cms/CmsExerciseLab'));
...
function AppShell() {
  ...
  const handleLogin = async (email, password, otp = null, turnstileToken = null) => {
    const res = await fetch(`${API_BASE}/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, password, otp, turnstile_token: turnstileToken }),
    });
    ...
    if (!res.ok) {
      ...
      if (res.status === 403 && dataErr?.detail?.requires_captcha) {
        ...
        e.captchaUntil = dataErr?.detail?.captcha_until || null;
        e.captchaRemaining = dataErr?.detail?.captcha_remaining ?? null;
        throw e;
      }
      if (res.status === 429 && dataErr?.detail?.locked) {
        const e = new Error(dataErr?.detail?.message || 'Too many attempts');
        e.locked = true;
