...
function parseHexColor(input) {
  ...
  const rr = parseInt(r, 16); const gg = parseInt(g, 16); const bb = parseInt(b, 16);
...
function parseRgbColor(input) {
  ...
  if (!m) return null;
  const parts = m[1].split(",").map((x) => x.trim()).slice(0, 3).map((x) => Number(x));
...
function pickThemeVars(pageBg, isGradient) {
  if (isGradient) {
    return {
      "--hl-card-bg": "rgba(255,255,255,0.78)", "--hl-card-border": "rgba(15,23,42,0.10)",
      "--hl-text": "#0f172a", "--hl-muted": "rgba(15,23,42,0.65)", "--hl-soft": "rgba(15,23,42,0.08)",
  ...
  if (isDark) {
    return {
      "--hl-card-bg": "rgba(15,23,42,0.62)", "--hl-card-border": "rgba(248,250,252,0.14)",
      "--hl-text": "#f8fafc", "--hl-muted": "rgba(248,250,252,0.72)", "--hl-soft": "rgba(248,250,252,0.14)",
...
// Resolves any media URL to an absolute URL.
// Handles /static/... (backend-hosted), /banners/... (FE preset), relative, and absolute.
function resolveUrl(url) {
  ...
  // Backend-hosted paths always need the API_BASE prefix
  if (s.startsWith("/static/") || s.startsWith("static/")) return `${API_BASE}${s.startsWith("/") ? s : "/" + s}`;
  if (s.startsWith("/uploads/") || s.startsWith("uploads/")) return `${API_BASE}${s.startsWith("/") ? s : "/" + s}`;
  // Frontend preset banners (served from Vercel/FE, not backend)
  if (s.startsWith("/banners/") || s.startsWith("banners/")) return s.startsWith("/") ? s : `/${s}`;
  // Any other absolute path — keep as-is (relative to current origin)
...
async function fetchJsonOrThrow(url, init) {
  ...
  if (!res.ok) {
    let hint = "";
    try { hint = (await res.text()).slice(0, 120); } catch {}
    throw new Error(`Request failed (${res.status})${hint ? `: ${hint}` : ""}`);
  }
  if (!ct.includes("application/json")) {
    const txt = await res.text();
    throw new Error(`Expected JSON but got ${ct || "unknown"}: ${txt.slice(0, 120).replace(/\s+/g, " ")}`);
  }
  return res.json();
}
...
function StatCard({ label, value }) {
  return (
    <div className="rounded-3xl border px-5 py-4 shadow-sm backdrop-blur" style={{ background: "var(--hl-card-bg)", borderColor: "var(--hl-card-border)" }}>
      <div className="text-xs tracking-wide" style={{ color: "var(--hl-muted)" }}>{label}</div>
      <div className="mt-1 text-2xl font-semibold" style={{ color: "var(--hl-text)" }}>{value}</div>
    </div>
  );
}
...
function Card({ className = "", children }) {
  return (
    <div className={`rounded-3xl border shadow-sm backdrop-blur ${className}`} style={{ background: "var(--hl-card-bg)", borderColor: "var(--hl-card-border)" }}>
      {children}
    </div>
  );
}
...
function Button({ variant = "primary", disabled, onClick, children }) {
  const base = "inline-flex items-center justify-center rounded-2xl px-4 py-2 text-sm font-semibold transition disabled:opacity-60 disabled:cursor-not-allowed";
  if (variant === "neutral") {
    return (
      <button disabled={disabled} onClick={onClick} className={base} style={{ color: "var(--hl-text)", background: "rgba(255,255,255,0.18)", border: "1px solid var(--hl-soft)" }}>
        {children}
      </button>
    );
  }
  return (
    <button disabled={disabled} onClick={onClick} className={`${base} bg-orange-500 hover:bg-orange-600 text-white shadow-sm`}>
      {children}
    </button>
  );
}
...
export default function PublicUserPage({ token }) {
  ...
  // Banner: check every possible field the backend might return it under
  const rawBanner =
    profile?.banner_url ||
    data?.banner_url ||
    ...
    "";
  const bannerUrl = resolveUrl(rawBanner);

  // Avatar: check every possible field
  const rawAvatar =
    profile?.avatar_url ||
    profile?.avatar ||
    data?.avatar_url ||
  ...
  const topFriends = Array.isArray(data?.top_friends) ? data.top_friends
    : Array.isArray(profile?.top_friends) ? profile.top_friends
    : Array.isArray(data?.friends_preview) ? data.friends_preview
    : [];

  const relationship = String(data?.friendship || profile?.friendship || "none").toLowerCase();
  const friendRequestId = data?.friend_request_id ?? profile?.friend_request_id ?? data?.friendship?.request_id ?? data?.friendship?.id ?? null;
  ...
  const friendsPublic =
    typeof profile?.friends_public === "boolean" ? profile.friends_public
    : typeof data?.friends_public === "boolean" ? data.friends_public
    : true;
  ...
  const heroCta = useMemo(() => {
    ...
    if (friendStatus === "incoming_pending") return <Button disabled={actionBusy} onClick={() => friendAction("accept")}>Confirm request</Button>;
    if (friendStatus === "outgoing_pending") return <Button variant="neutral" disabled>Request sent</Button>;
    if (friendStatus === "friends") return <Button variant="neutral" disabled={actionBusy} onClick={() => friendAction("remove")}>Friends ✓</Button>;
    return <Button disabled={actionBusy} onClick={() => friendAction("request")}>Add friend</Button>;
            ...
            <>
                        ...
                        {avatarUrl ? (
                          <img
                            ...
                            onError={(e) => {
                              ...
                              e.currentTarget.parentElement.innerHTML = `<div class="w-full h-full grid place-items-center text-2xl font-bold text-orange-600">${(data.name || data.username || "?")[0]?.toUpperCase()}</div>`;
                            }}
                          />
                        ) : (
                          <div className="w-full h-full grid place-items-center text-2xl font-bold text-orange-600">
              ...
              <div className="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-2 space-y-6">
                  {isPrivateView ? (
                    <Card className="p-6">
                      <div className="text-base font-semibold" style={{ color: "var(--hl-text)" }}>This profile is private</div>
                      <div className="mt-2 text-sm" style={{ color: "var(--hl-muted)" }}>Send a friend request to view learning stats and activity.</div>
                    </Card>
                  ) : (
                    <Card className="p-6">
                      <div>
                        <div className="text-lg font-semibold" style={{ color: "var(--hl-text)" }}>Recent learning activity</div>
                        <div className="text-sm" style={{ color: "var(--hl-muted)" }}>Exercises completed in the last 7 days</div>
                      </div>
                      {!activity?.days ? (
                        <div className="mt-4 text-sm" style={{ color: "var(--hl-muted)" }}>No activity data yet.</div>
                      ) : (
                ...
                <div className="space-y-6">
                  <Card className="p-6">
                    <div className="text-lg font-semibold" style={{ color: "var(--hl-text)" }}>Stats</div>
                  ...
                  {!isPrivateView && canSeeFriendsSection ? (
                    <Card className="p-6">
                      <div className="flex items-end justify-between gap-4">
                        <div>
                          <div className="text-lg font-semibold" style={{ color: "var(--hl-text)" }}>Friends</div>
                          <div className="text-sm" style={{ color: "var(--hl-muted)" }}>A quick peek at top friends</div>
                        </div>
                        <button onClick={() => navigate("/friends")} className="text-sm font-semibold hover:opacity-90" style={{ color: "var(--hl-text)" }}>
                          View all
                        </button>
                      </div>

                      {topFriends.length === 0 ? (
                        <div className="mt-4 text-sm" style={{ color: "var(--hl-muted)" }}>No friends yet.</div>
                      ) : (
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center gap-3">
                                  <div className="w-10 h-10 rounded-xl overflow-hidden grid place-items-center flex-shrink-0 bg-gradient-to-br from-orange-400 to-red-500" style={{ border: "1px solid var(--hl-soft)" }}>
                                    {fAvatar ? (
                                      <img src={fAvatar} alt="" className="w-full h-full object-cover"
                                        onError={(e) => { e.currentTarget.style.display = "none"; }}
                                      />
                                    ) : (
                                      <span className="text-white text-sm font-bold">{(f.name || f.username || "?")[0]?.toUpperCase()}</span>
                                    )}
                                  </div>
                                  <div className="min-w-0 flex-1">
                                    <div className="font-semibold truncate" style={{ color: "var(--hl-text)" }}>{f.name || f.username}</div>
                                    <div className="text-sm truncate" style={{ color: "var(--hl-muted)" }}>@{f.username}</div>
                                  </div>
                                  <div className="text-sm" style={{ color: "var(--hl-muted)" }}>{fXp} XP</div>
                                </div>
                              </button>
                            );
                          })}
          ...
          return (
            <div key={x.key} className="flex flex-col items-center gap-2 flex-1">
              <div className="w-full max-w-[46px]">
                <div className="relative h-20 w-full rounded-2xl overflow-hidden border" style={{ background: "rgba(255,255,255,0.10)", borderColor: "var(--hl-soft)" }}>
                  <div
                    className="absolute bottom-0 left-0 right-0 rounded-2xl"
                    style={{ height: `${allZero ? 8 : Math.max(8, h)}px`, background: "linear-gradient(180deg, rgba(252,114,41,.95), rgba(252,76,48,.75))" }}
                    title={`${x.label}: ${x.v}`}
                  />
                </div>
              </div>
              <div className="text-[11px]" style={{ color: "var(--hl-muted)" }}>{x.label}</div>
            </div>
          );
        })}
      </div>
