...
import { Save, RotateCcw, MessageCircle, Volume2, ListChecks, Check, Map as MapIcon } from "lucide-react";
...
export default function CmsAdventures() {
  ...
  const [drafts, setDrafts] = useState({});   // { [advId]: draft }
  ...
  async function save(advId) {
    setBusy(advId);
    try {
      await api.saveAdventureOverride(advId, draftToOverride(drafts[advId]));
              ...
              <div key={base.id} className="rounded-3xl bg-white p-5 ring-1 ring-slate-200 shadow-sm space-y-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="flex items-center gap-1 text-xs font-semibold text-slate-400"><MapIcon className="h-3.5 w-3.5" /> {base.map.width}×{base.map.height} scene · id "{base.id}"</div>
                  </div>
